...
var countdown = {
  ...
  tick: function() {
    ...
    $('.countdown-time').html(countdown.getString());
    // console.log(countdown.getString());
    if (countdown.val <= 10) {
...
var sravnenie = {
  ...
  initBtn: function() {
    ...
    $_wizard_answer_btn.on(MOUSE_DOWN, function(e) {
      //получим инфу о шаге
         $('.module_comment').show("slow");
         $('.countdown-time').hide(300);
      var textcom = $(this).data('comment');
      var mas = textcom.split(' ');
      mas[0] = '<span class="open-sans-regular red-text">'+ mas[0];
      mas[5] = mas[5] + '</span> <span class="open-sans-lite-it module__answer-font_litel">';
      mas.push('</span>');
      var str= mas.join(' ');
      // console.log(mas);
            
         $('.module_comment').find('.module__answer-text').html(str);
      ...
      // $(this).closest('.module-answer').find('.wizard-btn-answer').addClass('hidden');

      $(this).addClass(correct ? 'btn--correct' : 'btn--no-correct');    ///поправил 
      
      if (!correct) {
        $('#step_' + step).find('.wizard-btn-answer[data-correct="1"]').addClass('btn--correct '); //поправил 
      }
      ...
      $('html, body').stop().animate({
        scrollTop: $(this).parents('.module-answer').next().offset()    //тут был еше .top который давал ошибку
      }, 1000);
      ...
      $('#pstep_' + step).removeClass('active-test').addClass(correct ? 'good-test' : 'error-test');   //поправил тут
  ...
  wizard: function(_step) {
    ...
    $_li_pagination
      .removeClass('active-test');
    $('#pstep_' + _step)
      
      .addClass('active-test');
    //for mobile
  ...
  showNextStep: function(_step) {
     $('.module_comment').hide(300);
     $('.countdown-time').show("slow");

    if (_step + 1 > count_steps_of_wizard) {
      _send = false;
       $('.wizard-btn-answer').addClass('hidden');
...
$(window).on('load', function() {
  if ($('.module-wizard--next').length > 0) {
    sravnenie.init();
  }
});
