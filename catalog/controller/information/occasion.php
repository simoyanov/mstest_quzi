	...
	public function view(){
		...
		foreach ($occasion_place_images as $occasion_place_image) {
			$data['occasion_place']['images'][] = array(
				...
				'image_full' => $this->model_tool_image->resize($occasion_place_image['image'], 1920,1280,'h'),
				'image' => $this->model_tool_image->resize($occasion_place_image['image'], 950,450,'h')
			);
		}
		//получим метро
		...
		foreach ($occasion_images as $occasion_image) {
			$data['occasion']['images'][] = array(
				...
				'image_full' => $this->model_tool_image->resize($occasion_image['image'], 1920,1280,'h'),
				'image' => $this->model_tool_image->resize($occasion_image['image'], 950,450,'h')
			);
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/occasion.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/information/occasion.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/information/occasion.tpl', $data));
		}
