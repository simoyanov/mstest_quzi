...
var body_size = $('body');
var centrer_frame = $('#center_frame');
var time = 500;
var ratio = 0.562;
var frame_w, frame_h, frame_d, centrer_frame_w, centrer_frame_h, centrer_frame_t, centrer_frame_l;
var $load_frame = $('#load_frame');
...
var visual = {
    init: function() {
        ...
        if (mobile) {
            body_size.addClass('mobile');
        } else {
            body_size.addClass('no-mobile');
        }
        ...
        /* ---------------------------------------------- /*
         * Show/Hide overlay menu rewrite
        /* ---------------------------------------------- */
        $(document).on(MOUSE_DOWN, '#toggle-menu', function(e) {
            e.preventDefault();
            visual.showMenu();
            $('body').addClass('aux-navigation-active');
        });
        $(document).on(MOUSE_DOWN, '#overlay-menu-hide', function(e) {
            e.preventDefault();
            console.log('qeq');
            visual.hideMenu();
    ...
    moduleInit: function() {
        ...
        modules.each(function() {
            ...
            if ($(this).hasClass('module-full-height')) {
                if ($(window).height() > min_window_h) {
                    $(this).height($(window).height(min_window_h - 120));
                } else {
        ...
        if ($(window).height() > min_window_h) {
            $('.block-h-hero').height($(window).height());
        } else {
            $('.block-h-hero').height(min_window_h);
        }
    },
    initHome:function(){
        $('.module-achievement__test-image').each(function() {
            var _wmach = $(this).width();
            $(this).find('img').next().width(_wmach*.7);
            console.log(_wmach);
            //$('.module-achievement__test-image_title').width(_wmach);
        })
    ...
    scale_show: function(targ, delay, time) {
        targ.css({
            'opacity': 0,
            'scale': 0.8
    ...
    scale_hide: function(targ, delay, time) {
        targ.delay(delay).transition({
            'opacity': 0,
            'scale': 0.8
        }, time);
    },
    element_check_size: function(targ) {
        targ.css({
            'width': centrer_frame_w,
            'height': centrer_frame_h,
            'top': centrer_frame_t,
            'left': centrer_frame_l
        });
    },
    ...
    scaleContent: function() {
        ...
        if (frame_d > ratio) {
            centrer_frame_w = frame_h / ratio;
            centrer_frame_h = frame_h;
            centrer_frame_t = 0;
            centrer_frame_l = 0.5 * (frame_w - centrer_frame_w);
        } else {
            centrer_frame_w = frame_w;
            centrer_frame_h = frame_w * ratio;
            centrer_frame_t = 0.5 * (frame_h - centrer_frame_h);
            centrer_frame_l = 0;
        }
        visual.element_check_size(centrer_frame);
    },
    animate:function(_element, animation){
        if(animate_el){
            $(_element).addClass(animation);
            animate_el = false;
        }else{
            $(_element).removeClass(animation) ;
            animate_el = true;
        }
    ...
    animateElements:function(){
        setInterval(function() {
          visual.animate('.card--news','animated tada');
        }, 2000);  
    }
};

var count_card = 0;
var json_achievement = [];
var raiting = {
  init:function(){
   
    if(!mobile){
      raiting.nomobileRatingAction();
    }else{
      raiting.mobileRatingAction();
    }

    $(document).on(MOUSE_DOWN,'#send_result_raiting',function(e){
      e.preventDefault();
    ...
    $(document).on(MOUSE_DOWN,'#send-result-mobile',function(e){
       e.preventDefault();
       raiting.sendResult();
    });
    $(document).on(MOUSE_DOWN,'#popup-mobile-hide',function(e){
       e.preventDefault();
       $('.popup-mobile-action').removeClass('active');
    });
    $('.preaction-loader').removeClass('hidden');
    $(document).on(MOUSE_DOWN,'#preaction-hide',function(e){
       e.preventDefault();
       $('.preaction-loader').addClass('hidden');
    });
    
  },
  sendResult:function(){
    var data = $('#achievements-form').serializeArray();  
    console.log(data);
    var url = '/iwanttovote'
        $.ajax({
          url: url,
          ...
          success: function(obj) {
            ...
            if(obj['success']){
              ...
              yaCounter31626893.reachGoal(action_cl_rbtn_ya);
  ...
  mobileRatingAction:function(){
    $(document).on(MOUSE_DOWN,'.chose-achievement-mobile',function(){
      console.log($(this).data('item'))
      var id_item = $(this).data('item');
      if(count_card < 5){
         $('#'+id_item).addClass('item-select');
        $(this).addClass('hidden').next().removeClass('hidden');
        count_card ++;
        raiting.actionForMobile();
      }
     
    });
    $(document).on(MOUSE_DOWN,'.chose-achievement-mobile-disable',function(){
      console.log($(this).data('item'))
      var id_item = $(this).data('item');
      $('#'+id_item).removeClass('item-select');
      $(this).addClass('hidden').prev().removeClass('hidden');
      count_card--;
    });
    
  },
  nomobileRatingAction:function(){
    ...
    $('#achievements-gallery').owlCarousel({
      ...
      itemsMobile : false,
      pagination  : false,
      afterMove: raiting.scrollGallery,
      afterUpdate:raiting.resize
    });
    
    
    $(document).on(MOUSE_DOWN,'#larrow',function(e){
      e.preventDefault();
       $('#achievements-gallery').trigger('owl.prev');
       raiting.scrollGallery
    });
    $(document).on(MOUSE_DOWN,'#rarrow',function(e){
      e.preventDefault();
      $('#achievements-gallery').trigger('owl.next');
      raiting.scrollGallery
    });
    ...
    $('.owl-item').draggable({
      revert: true,
      helper: raiting.helper,
      appendTo:'body',
      start: raiting.startDrag
    });
    $('.achievement-item').droppable( {
      accept: '.owl-item',
      hoverClass: 'hovered',
      drop: raiting.handleCardDrop,
    ...
    $(document).on(MOUSE_DOWN,'.gallery_item-action',function(e){
      $('#send_result_raiting').addClass('disabled');
      e.preventDefault();
      var achiev = $(this).parents('.achievement-item');
      achiev.droppable( 'enable' );
      console.log($(this).parents('.gallery_item-block').attr('id'))
      var draggable_item_id  = '#main_'+$(this).parents('.gallery_item-block').attr('id');
      $(draggable_item_id).parents('.owl-item').draggable( 'enable' );
      console.log($(draggable_item_id));
      $(this).parents('.gallery_item-block').remove();
      count_card--;
    });
  },
  actionForMobile:function(){
     if(count_card == 5){
         $('.item-select').each(function( index ) {
            console.log( 'item_id' + $( this ).find('.gallery_item-block').data('achievement') );
            item = {}
            item ["achievement_id"] = $( this ).find('.gallery_item-block').data('achievement');
            item ["raiting_id"] = $( this ).find('.gallery_item-block').data('position');
            json_achievement.push(item);
          });
         //показываем всплывашку
         $('.popup-mobile-action').addClass('active');
     }
  },
  actionForNoMobile:function(){
    if(count_card == 5){
      //собираемвсе ачивки
      json_achievement = [];
      ...
      console.log(json_achievement);
      //собрали массив/
      $('#send_result_raiting').removeClass('disabled');
    }else{
      $('#send_result_raiting').addClass('disabled');
    }
  },
  initAction:function(){
    if (mobile) {
      ...
      raiting.actionForNoMobile();
    }
  },
  handleCardDrop:function(event, ui ){
    //ui.draggable - элемен котрый таскаем
    //ui.draggable.addClass( 'correct' );
    count_card ++;
    ui.draggable.draggable( 'disable' );
    $(this).droppable( 'disable' );
    var id_item = $(ui.draggable).attr('id');
    var element=$(ui.draggable).find('.gallery_item-block').clone();
    element.data('id_number', id_item);
    $(this).append(element);
  ...
  scrollGallery:function(){
    ...
    var owl = $('#achievements-gallery').data('owlCarousel');
    console.log(owl.currentItem);
    console.log(owl.maximumItem);

    console.log(owl)
    if (owl.currentItem === owl.maximumItem) {
       $('#larrow').removeClass('hidden');
       $('#rarrow').addClass('hidden');
    } else if(owl.currentItem === 0){
       $('#rarrow').removeClass('hidden');
  ...
  resize:function(){
    if($('.achievements-bg').length > 0 && !mobile){
      var w_achievements = $('.achievements-bg').width();
      console.log(w_achievements);
      var w_item = (w_achievements)/5;
      $('.achievement').css({'width': w_item});
      var heights = $(".gallery_item-heading").find('p').map(function (){
          return $(this).height();
      }).get(),
      maxHeight = Math.max.apply(null, heights);
      $('.gallery_item-heading').css({'height':maxHeight+30});
    }

    if($('.achievements-gallery').length > 0 && !mobile){
      ...
      maxHeight = Math.max.apply(null, heights);
      $('.gallery_item-heading').css({'height':maxHeight+30});
      _heightGalleryItem = $('.owl-item').height();
      _widthGalleryItem  = $('.owl-item').width();
      $('.left-arrow').css({'width':_widthGalleryItem/2,'height':_heightGalleryItem});
      $('.right-arrow').css({'width':_widthGalleryItem/2,'height':_heightGalleryItem});
    }
    if($('.achievements').length > 0 && !mobile){
      var w_achievements = $('.achievements').width();
      console.log(w_achievements);
      var w_item = (w_achievements)/5;
      var h_item = $('.achievements-gallery_item').height();
      $('.achievement').css({'width': w_item,'height': h_item});
    }
    if($('.my-achievements').length > 0 && !mobile){
      var w_achievements = $('.my-achievements').width();
      console.log(w_achievements);
      var w_item = (w_achievements)/5;
      //var h_item = $('.achievements-gallery_item').height();
      $('.achievement').css({'width': w_item}); 
...
var count_item = 0;
var current_correct_item = 0;
var current_no_correct_item = 0;
var current_other_item = 0;
var current_correct_item = 0;
//для блокировки постоянного нажатия м и отправиa ajax
var quiz_active = true;
var send_data = [];
//simple - простой тест вопросы по порядку
//medium - посложнее вопросы можно выбирать
var _data = [];
var step = {
	init:function(){
    step.initStep();
    ...
    //покажем подсказку
    if(type == 'medium'){
       $('.preaction-loader-test').removeClass('hidden')
    }
   
    //получим количество шагов
    count_step = $(document).find('.wizard').find('.pagination').find('li').length;
    $('#all_question').html(count_step);
    //включим первый шаг
    //количестов коректных выбранных count_correct
    step.wizard(1);
  },
  initBtnStep:function(){
    $('.answer-btn').on( MOUSE_DOWN, function(e){
      e.preventDefault();
        var current_step  = $(this).data('step');
        var voice         = $(this).data('voice');
        var achievement   = $(this).data('achievement');
        var answer_value  = $(this).data('answer')

        $('#pstep_'+current_step).attr('class', '').removeClass('active').addClass(voice);
        $('#step_'+current_step).find('.avatar').attr('class', '').addClass('avatar').addClass(voice);

        if(quiz_active){
          //добавить проверку на сушествует ли уже такой елемент
          $('#poll-form').empty();
          $('#poll-form').append('<input type="hidden" name="quiz_id" value="'+ q +'">');
          current_correct_item = 0;
          current_no_correct_item = 0;
          current_other_item = 0;
          count_item = 0;
          $(document).find('.pagination').find('li').each(function( index ) {

            //вынести бы в соответсвиие в конифиг
            if($(this).hasClass('correct')){
              count_item +=1;
              current_correct_item +=1;
              $('#poll-form').append('<input type="hidden" name="answer['+ $( this ).data('achievement') +']" value="1">');
            }
            if($(this).hasClass('no-correct')){
              count_item +=1;
              current_no_correct_item +=1;
              $('#poll-form').append('<input type="hidden" name="answer['+ $( this ).data('achievement') +']" value="0">');
            }
            if($(this).hasClass('other')){
              count_item +=1;
              current_other_item +=1;
              $('#poll-form').append('<input type="hidden" name="answer['+ $( this ).data('achievement') +']" value="2">');
            }
          });
          $('#current_count').html(current_correct_item + ' ' + step.decOfNum(current_correct_item, ['чиновник', 'чиновника', 'чиновников']) );
          step.showNextStep(current_step);
          };
          
      });
      $('#send_result_quiz_medium').on(MOUSE_DOWN,function(e){
        e.preventDefault();
        if(!$(this).hasClass('disabled')){
          quiz_active = false;
          step.sendResult();
       ...
       $('#mobile_send_result_quiz_medium').on(MOUSE_DOWN,function(e){
        e.preventDefault();
        if(!$(this).hasClass('disabled')){
          quiz_active = false;
          step.sendResult();
      ...
      $('.pagination-action').find('a').on(MOUSE_DOWN,function(e){
        e.preventDefault();
       ...
       $(document).on(MOUSE_DOWN,'#preaction-hide-loader-test',function(e){
         e.preventDefault();
         $('.preaction-loader-test').addClass('hidden');
      });

      $(document).on(MOUSE_DOWN,'#next-step',function(e){
         e.preventDefault();
         var current_step = $('.pagination').find('.active').find('a').data('step')
      ...
      $(document).on(MOUSE_DOWN,'#prev-step',function(e){
         e.preventDefault();
         var current_step = $('.pagination').find('.active').find('a').data('step')
         console.log(current_step);
         var prev_step = current_step-1;
         if(current_step == 1){
          prev_step = count_step;
         }
         step.wizard(prev_step);
      });

  },
	showNextStep:function(_step){
		if(type == 'simple'){
      if (_step+1 > count_step) {
        quiz_active = false;
        step.endWizard();
      }else{
        step.wizard(_step+1);
      ...
      console.log('all_count            ->>>'+count_item);
      console.log('current_correct_item ->>>'+current_correct_item);
      console.log('current_no_correct_item ->>>'+current_no_correct_item);
      console.log('current_other_item ->>>'+current_other_item);
      console.log('count_step           ->>>'+count_step)
      console.log('vis_condition        ->>>'+(current_correct_item <= count_correct && current_correct_item > 0));
      if ( count_item == count_step) {
        if( 
            (current_correct_item <= count_correct && current_correct_item > 0)  || 
            (current_no_correct_item == count_step && current_correct_item == 0) || 
            (current_other_item == count_step && current_correct_item == 0) || 
            current_correct_item == count_step
        ){
          $('#send_result_quiz_medium').removeClass('disabled');
          if(mobile){
            $('#mobile_send_result_quiz_medium').removeClass('disabled');
          }
       }else{
          $('#send_result_quiz_medium').addClass('disabled');
          if(mobile){
            $('#mobile_send_result_quiz_medium').addClass('disabled');
          }
       }
      }
        if(_step == count_step){
	...
	wizard:function(_step){
		$(document).find('.step_view').removeClass('current-step');
		$('#step_'+_step).addClass('current-step');
    $(document).find('.pagination li').removeClass('active');
		$('#pstep_'+_step).removeClass('disabled').addClass('active');
		//for mobile
...
var battle = {
	init:function(){
		battle.initBattle();
		//ion.sound.play("snap");
		battle.initBtnBattle();
	},
	initBattle:function(){
	   $('.battle').find('.animate-element').each(function(index){
			console.log(index);
			visual.scale_show($(this),time+300*index,350)	
		})
    $('.hat_1').css({'display':'none'}).transition({x:'-200px'});
    $('.hat_2').css({'display':'none'}).transition({x:'200px'});
	    ion.sound({
            sounds: [
                {
                	name: "start",
	    ...
	    //setTimeout(function(){ion.sound.play("start");},500);
	    if(!mobile){
	    	$('#fighter_1')
        .on('mouseenter', function() { 
        //	ion.sound.play("hover");
        	$('.hat_1').css({'display':'block'}).transition({x:'0px'},250);
        })
        .on('mouseleave', function() {
        //	ion.sound.stop("hover");
        	$('.hat_1').transition({x:'-200px'},250);
        });

        $('#fighter_2')
        .on('mouseenter', function() { 
        //	ion.sound.play("hover");
        	$('.hat_2').css({'display':'block'}).transition({x:'0px'},250);
        })
        .on('mouseleave', function() {
        //	ion.sound.stop("hover");
        	$('.hat_2').transition({x:'200px'},250);
        });
	    }
        
  	},
  	initBtnBattle:function(){
  		$('#start_game').on(MOUSE_DOWN,function(e){
	        e.preventDefault();
					battle.launchStartScreen();
          $('.preaction-battle').remove();
	    });
	    $('.battle-strike').on( MOUSE_DOWN, function(e){
	      e.preventDefault();
	        var current_step  = $(this).data('step');
	        var voice         = $(this).data('voice');
	        var achievement   = $(this).data('achievement');
	        var answer_value  = $(this).data('answer')
	        battle.strike(answer_value);
	        $('#pstep_'+current_step).attr('class', '').removeClass('active').addClass(voice);
	        $('#step_'+current_step).addClass('hidden').find('.avatar').attr('class', '').addClass('avatar').addClass(voice);
	        ...
	        if(quiz_active){
	          ...
	          battle.showNextStep(current_step);
	          };
	          
	      });
  	},

  	strike:function(direction){
  		var between = $('.count-time').width();
  		var _w_fighter = $('.module__fighter__image').width()
  		var distance = _w_fighter+between;
      if(mobile){
        distance = 150;
      }
      console.log(distance);
  		//direction - id от кого отлетает удар
  		var name_sound = "fighter_"+ direction +"_" + (Math.floor(Math.random() * 5)+1);
  		console.log(name_sound);
  		ion.sound.play(name_sound);
  		if(direction == '1'){
  			fighter_2 += 1;
  			$('#fighter_2').css({'z-index':10});
  			$('#fighter_1')
  				.css({'z-index':20})
  				.next()
  				.css({'opacity':0})
  				.delay(100)
  				.transition({'opacity':1, x: distance+'px' }, 250 ,function(){
	  				//трясем аватарку противника
	  				$('#fighter_2')
	  					.css({x:50})
	  					.transition(
	  						{x:0}, 350, 'easeInOutQuint',
	  						function(){
	  							$('#fighter_1').next().css({'opacity':0}).transition({x: '0px' });
	  							if ( !(fighter_2 & 1) ) {
	  								$(this).find('img').removeClass('current')
	  								$(this).find('img').each(function() {
												if ($(this).attr('data-strike') == fighter_2) {
  		...
  		if(direction == '2'){
  			fighter_1 += 1;
  			$('#fighter_1').css({'z-index':10});
  			$('#fighter_2').css({'z-index':20})
  				.next()
  				.css({'opacity':0})
  				.delay(100)
  				.transition({'opacity':1, x: '-' + distance +'px' }, 250, function(){
  				...
  				$('#fighter_1')
  					...
  					.transition(
  						{x:0}, 350, 'easeInOutQuint',
  						function(){
  							$('#fighter_2').next().css({'opacity':0}).transition({x: '0px' });
  							if ( !(fighter_1 & 1) ) {
  								$(this).find('img').removeClass('current')
  								$(this).find('img').each(function() {
											if ($(this).attr('data-strike') == fighter_1) {
												$(this).addClass('current')
											}
									});
  							}
  		...
  		console.log('fighter_1 ->> ' + fighter_1);
  		console.log('fighter_2 ->> ' + fighter_2);
  	},
  	wizard:function(_step){
			$(document).find('.round_view').removeClass('current-step');
			$('#step_'+_step).addClass('current-step');
			$(document).find('.pagination li').removeClass('active');
			$('#pstep_'+_step).removeClass('disabled').addClass('active');
			$(document).find('.count_round').html(_step);
			//for mobile
			$('.mobile-pagination').find('.active').html(_step);
			battle.resize();
  	...
  	launchStartScreen:function(){
			//ion.sound.stop("start");
			//ion.sound.play("fight");
		  //ion.sound.play("battle");
			 
			 //получим количество шагов
			count_step = $(document).find('.battle').find('.pagination').find('li').length;
			$('.count-time').find('img').delay(250).css({'display':'none'});
			$('.time-round').css({'opacity':0,'scale':0.5,'display':'block'}).delay(250).transition({'scale':1,'opacity':1},250);
			$('.module-subtitle').css({'opacity':0,'scale':0.5,'display':'block'}).delay(250).transition({'scale':1,'opacity':1},250);
		    battle.wizard(1);
		...
		showNextStep:function(_step){
		    if (_step+1 > count_step) {
		      quiz_active = false;
		      setTimeout(function(){battle.endWizard();},500); 
		      
		    }else{
		      setTimeout(function(){battle.wizard(_step+1);},500); 
		    }
		...
		resize:function(){
			var _width_strike = $('.current-step').find('.col-xs-6').width();
			var _width_img_strike = $('.current-step').find('.pic-strike').width();
      var w = _width_strike-_width_img_strike;
      var _heights = $('.current-step').find('.pic-strike').height();
      if(mobile){
        w = '100%';
        _heights = 'auto';
      }
        $('.current-step').find('.name-strike').css({'height':_heights});
			$('.current-step').find('.name-strike').css({'width':w});
 		...
 		sendResult:function(){
	    var data = $('#poll-form').serializeArray(); 
      yaCounter31626893.reachGoal('rbtn_quiz_battle_mer');
	    console.log(data);
	    var url = '/send_result_quiz'
	       $.ajax({
	          url: url,
...
var action = {
	init:function(){
        ...
        $(document).on(MOUSE_DOWN,'#send_result_question',function(e){
            e.preventDefault();
            action.sendAddQuestion();
            yaCounter31626893.reachGoal('add_the_object');
        })
    },
    sendAddQuestion:function(){
        var data = $('#add-form').serializeArray();  
        var url = '/upload_achiviement'
        $.ajax({
            url: url,
            type: 'POST',
            data: data,
...
$(document).ready(function() {
	...
	if($('.achievements').length > 0 || $('.my-achievements').length > 0 || $('.achievements-bg').length > 0){
	  	setTimeout(function(){raiting.resize();},500);
		setTimeout(function(){raiting.resize();},1500);
		setTimeout(function(){raiting.resize();},2500);
	}
	if($('.wizard').length > 0){
		step.init();
	}
	...
	if($('.module-achievement__test-image').length > 0 ) {
		visual.initHome();
	}
	social.init();
